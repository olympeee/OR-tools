...
namespace Sudoku.GeneticAlgo
{
    public class GeneticAlgorithmSolverPerm : ISudokuSolver
    {
        public SudokuGrid Solve(SudokuGrid s)
        {
            var toSolveSudoku = new SudokuBoard(s.Cells.Flatten()); ///int[][].Flatten() -> transform an int[][] to an int[]

            var populationSize = 500;

            var sudokuChromosome = GetChromosome(toSolveSudoku);

            var fitness = new SudokuFitness(toSolveSudoku);
    ...
    public class GeneticAlgorithmSolverRandomPerm : GeneticAlgorithmSolverPerm
    {
        protected override ISudokuChromosome GetChromosome(SudokuBoard sBoard)
        {
            return new SudokuRandomPermutationsChromosome(sBoard, 10, 10);
        }
    }

    public class GeneticAlgorithmSolverCell : GeneticAlgorithmSolverPerm
    {
        protected override ISudokuChromosome GetChromosome(SudokuBoard sBoard)
        {
